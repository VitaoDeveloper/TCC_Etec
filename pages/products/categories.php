...
$categories = $pdo->query('SELECT c.id, c.name, c.description, COUNT(p.id) AS total_products FROM categories c LEFT JOIN products p ON p.category_id = c.id GROUP BY c.id, c.name, c.description ORDER BY c.name')->fetchAll();
?>
